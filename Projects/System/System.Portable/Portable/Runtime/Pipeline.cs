...
namespace System.Portable.Runtime {
    ...
    /// </summary>
    public class Pipeline : ITransientDependency {
        public Pipeline(IReflector reflector) {
            Actions = new HashSet<RegisteredAction>();
            Filters = new HashSet<RegisteredFilter>();
            Reflector = reflector;
        }
        ...
        public async Task Process<T>(T tObject) {
            await this.AsTask(o => {
                if (!(bool)Reflector.InvokeReturn(CreateFilter(tObject))) return;
                Reflector.InvokeAction(CreateAction(tObject));
            });
        }
        ...
        protected void ExecuteAction(WrappedAction action, object tObject) {
            Reflector.InvokeAction(action, tObject);
        }

        protected bool ExecuteFilter(WrappedFilter wrappedFilter, object tObject) {
            return (bool)Reflector.InvokeReturn(wrappedFilter, tObject);
        }
    }
}