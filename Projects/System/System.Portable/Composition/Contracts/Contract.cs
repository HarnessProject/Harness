...
namespace System.Composition.Contracts {
    public static class Contract {
        ...
        private static ValidationResult Assert<T>(this T o, string targetName, Delegate assertion, object[] args = null, string invalidMessage = null, bool thrw = false) {
            ...
            if (assertion.Try(a => (bool) Provider.Get<IReflector>().Invoke(a, arguments.ToArray()))
        ...
        public static bool Validate<T>(this T o) {
            var t = typeof (T);
            ...
            t.GetProperties()
                ...
                .SelectMany(
                    x =>
                        ...
                        .SelectMany(y => y.Assert(x, x.GetValue(o, null)))
                ).AddTo(results);
            return results.IsObjectValid;
        }
    }
}