...
namespace Harness.Framework {
    public static class TaskExtensions {
        public static Task EachAsync<T>(this IEnumerable<T> collection, Action<T> action) {
            return new Task(
                () => Task.WaitAll(collection.Select(i => Task.Factory.StartNew(() => action(i))).ToArray())
                );
        }

        public static Task EachAsync<T, TY>(this IEnumerable<T> collection, Action<T, TY> action, TY context) {
            return new Task(
                () => Task.WaitAll(collection.Select(i => Task.Factory.StartNew(() => action(i, context))).ToArray())
                );
        }
            ...
            return new Task(
                () =>
                    collection.EachAsync(
                        i => actions.EachAsync(a => a(i)).Await()
                    ).Await()
                );
        }
