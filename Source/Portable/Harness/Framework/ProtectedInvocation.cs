...
namespace Harness {
    public class ProtectedInvocation<T, TY> {
        ...
        public ProtectedInvocation<T, TY> Catch<TX>(Func<T, TX, TY> tCatch) where TX : Exception {
            if (!_exceptionFuncs.ContainsKey(_indexSeed))
                _exceptionFuncs[_indexSeed] = new Dictionary<Type, Func<T, Exception, TY>>();
        ...
        public TY Invoke() {
            foreach (var f in _protectedFuncs.OrderBy(x => x.Key))
                try {
                    ...
                    if (_exceptionFuncs[f.Key].ContainsKey(ext))
                        r = _exceptionFuncs[f.Key][ext](t, ex);

                    r = _exceptionFuncs[f.Key].ContainsKey(typeof (Exception))
                        ? _exceptionFuncs[f.Key][typeof (Exception)](t, ex)
                        : r;

                    if (_exceptionFuncs[f.Key].ContainsKey(ext))
                        _finallyFuncs[f.Key](r);
