...
namespace Harness {
    public class RegistrationContext : IRegistrationContext {
        ...
        private readonly
            Dictionary
                <Type,
                    List<Action<IRegistrationBuilder<object, ConcreteReflectionActivatorData, SingleRegistrationStyle>>>
                    > _handlers =
                        new Dictionary
                            <Type,
                                List
        ...
        public void RegisterHandlerForType<T>(
            Action<IRegistrationBuilder<T, ConcreteReflectionActivatorData, SingleRegistrationStyle>> registration) {
            Type t = typeof (T);
        ...
        public
            IEnumerable<Action<IRegistrationBuilder<object, ConcreteReflectionActivatorData, SingleRegistrationStyle>>>
            HandlersFor(Type t) {
            return
                from key in _handlers.Keys
                where key.IsAssignableFrom(t)
                from h in _handlers[key]
                select h;
        }
    }
}