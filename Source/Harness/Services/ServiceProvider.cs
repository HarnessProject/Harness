...
namespace Harness.Services {
    public abstract class ServiceProvider<T> : IServiceProvider<T> {
        ...
        public EvaluationResult<T> Evaluate<TY>(TY context) {
            ...
            List<IRequirement> contextualRequirements = Requirements.Where(requirement => (requirement.Type.IsAssignableFrom(contextType))).ToList();
            return !contextualRequirements.Any() || contextualRequirements.All(requirement => requirement.Evaluate(context)) ? new EvaluationResult<T> {IsMatch = true, Priority = Priority, Service = Service} : new EvaluationResult<T> {IsMatch = false};
        }
