...
namespace Harness {
    public abstract class EnvironmentBase<T> : IEnvironment {
        protected EnvironmentBase(bool buildContainer = true, Func<ContainerBuilder> containerBuilder = null) {
            ContainerBuilder builder = containerBuilder != null ? CreateContainer(containerBuilder) : CreateContainer(() => new ContainerBuilder());
        ...
        public ContainerBuilder CreateContainer(Func<ContainerBuilder> deferedBuilder) {
            ...
            iComponents.EachAsync(
                (component, c) =>
                    Activator.CreateInstance(component).As<IComponentRegistrationService<T>>().AttachToRegistration(c), 
                    context
                ).Await();

            iModules.EachAsync((type, b) => b.RegisterModule(Activator.CreateInstance(type) as IModule), builder).Await();

            iDependencies.EachAsync((type, c) => {
