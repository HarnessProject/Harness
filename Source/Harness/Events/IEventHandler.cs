...
namespace Harness.Events {
    ...
    public class EventManager : IEventManager {
        private readonly List<KeyValuePair<Type, Action<dynamic>>> _handlers = new List<KeyValuePair<Type, Action<dynamic>>>();

        #region IEventManager Members

        public void Handle<T>(params Action<T>[] handlers) {
            foreach (var handler in handlers) _handlers.Add(new KeyValuePair<Type, Action<dynamic>>(typeof (T), handler as Action<dynamic>));
        }
        ...
        public void Trigger<T>(T eventObject) {
            ...
            foreach (var handler in handlers) handler.Handle(eventObject);

            foreach (var handler in _handlers.ToLookup(k => k.Key, v => v.Value)[typeof (T)]) handler(eventObject);
        }
